"""
The script checks the dependencies of the current project and provides a Julia enviroment that
provides all current development versions of the QED dependencies.

The following steps are carried out
1. Create dependency graph of the active project without instantiating it. To do this, the QED
project is cloned and the current dev branch is used.
...
3. Calculates which packages must be added in which order for the QED package to be tested.
...
Return a list of dependencies which are defiend in the sctions `deps` and `extras` in a `Project.toml`
of a package.
...
QuantumElectrodynamics.jl `Project.toml`, it clones all directly and indirectly dependent QED
...
e.g. pkg_ordering[1] stands for the first round. The set contains all leafs that were found in the
...
Sorry for the difficult documentation. It is much easier to compare the input with the output of
the function to understand it.
