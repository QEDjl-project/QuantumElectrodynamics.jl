...
There is a last job, which checks if lines starting with `CI_UNIT_PKG_URL_` are exist in the commit message of the last commit. If they are exist, the test fails. This is required, because otherwise the merge code would be depend on non merged changes in sub packages and would be not compilable.
...
This section explains how the unit tests are prepared and executed. It is not mandatory to read the section if you only want to use the CI.
